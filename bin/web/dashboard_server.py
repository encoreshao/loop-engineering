...
_CHAT_ASSISTANT_SYSTEM_PROMPT = (
    ...
    "run-issue <url>. If the user's message contains or references a "
    "GitLab issue link (or asks you to work on/fix a specific issue by "
    "URL), call `chat-tool run-issue <that url>` - this starts a scoped, "
    "on-demand run of the loop for exactly that one issue, regardless of "
    "who it's assigned to. If you use one of the mutating actions "
